...
class ForensicEvidence:
    ...
    evidence_id: str = field(default_factory=lambda: hashlib.sha256(str(datetime.utcnow()).encode()).hexdigest()[:16])
    ...
    def add_custody_event(self, action: str, actor: str = "SYSTEM") -> None:
        ...
        self.chain_of_custody.append({
            ...
            "action": action,
            "actor": actor,
            "hash": hashlib.sha256(self.content).hexdigest() if self.content else self.content_hash
        })
...
class FraudIndicator:
    ...
    def risk_score(self) -> float:
        ...
        criminal_boost = sum(
            0.2 for v in self.statute_violations 
            if v.value[2] == Priority.CRITICAL
        )
    ...
    def from_dict(cls, data: dict) -> 'FraudIndicator':
        ...
        if 'statute_violations' in data:
            data['statute_violations'] = [
                ViolationType[v] if isinstance(v, str) else v 
                for v in data['statute_violations']
            ]
...
class ForensicDatabase:
    ...
    def _init_schema(self):
        ...
        cursor.execute("CREATE INDEX IF NOT EXISTS idx_analysis_risk ON analysis_results(risk_score)")
    ...
    def retrieve_evidence(self, evidence_id: str = None, content_hash: str = None) -> Optional[ForensicEvidence]:
        ...
        if row:
            # Decompress content
            content = lz4.frame.decompress(row["content_compressed"]) if row["content_compressed"] else b""
    ...
    def store_analysis(self, analysis_id: str, evidence_id: str, results: Dict) -> bool:
        ...
        try:
            ...
            """, (
                ...
                json.dumps([fi.to_dict() if hasattr(fi, 'to_dict') else asdict(fi) for fi in results.get("fraud_indicators", [])], default=str),
...
class MLFraudDetector:
    ...
    def _count_hedge_words(self, text: str) -> int:
        ...
        hedge_words = {
            'may', 'might', 'could', 'possibly', 'perhaps', 'appear',
            'suggest', 'seemingly', 'arguably', 'presumably', 'allegedly',
            'reportedly', 'uncertain', 'unclear', 'ambiguous'
        }
    ...
    def detect_financial_anomalies(self, financials: pd.DataFrame,
                                   peer_data: pd.DataFrame = None) -> List[FraudIndicator]:
        ...
        for column in ["Revenue", "Assets", "Liabilities"]:
            if column in financials.columns:
                ...
                if len(values) > 10:
                    ...
                    if first_digits:
                        observed_freq = pd.Series(first_digits).value_counts(normalize=True).sort_index()
                        benford_freq = pd.Series([np.log10(1 + 1/d) for d in range(1, 10)], index=range(1, 10))
                        
                        # Chi-square test
                        chi_stat = sum((observed_freq.get(i, 0) - benford_freq[i])**2 / benford_freq[i]
                                       for i in range(1, 10))
...
class SECDataAcquisition:
    ...
    async def __aenter__(self):
        ...
        headers = {
            "User-Agent": os.environ.get("SEC_USER_AGENT", "Forensic Analysis System forensic@investigation.local"),
                ...
                if response.status != 200:
                    ...
                    async with self.session.get(url_alt) as response2:
                        if response2.status != 200:
                            logger.error(f"Failed to download filing {accession_number}: {response.status} (tried {url})")
                            return None
...
class ForensicInvestigator:
    ...
    async def investigate_company(self, cik: str,
                                  years_back: int = 3,
                                 forms: List[str] = None,
                                 max_filings: int = 20) -> Dict[str, Any]:
        ...
        self.investigation_id = hashlib.sha256(
            f"{cik}_{datetime.utcnow()}".encode()
        ).hexdigest()[:16]
        ...
        try:
            async with SECDataAcquisition(self.db) as sec_client:
                ...
                logger.info(f"Analyzing {len(filings_to_analyze)} filings (user limit: {max_filings})")
                ...
                self.db.store_analysis(
                    ...
                    self.results.get("primary_evidence_id", ""),
                    self.results
                )
    ...
    async def _analyze_filing(self, filing: Dict):
        ...
        try:
            ...
            for indicator in fraud_indicators:
                indicator.evidence.append(f"Filing: {filing['form_type']} on {filing['filing_date']}")
        ...
        if filing.get("report_date"):
            ...
            if form_base in deadlines and delay_days > deadlines[form_base]:
                if "NT" not in filing["form_type"]:
                    self.results["fraud_indicators"].append(FraudIndicator(
    ...
    def _calculate_risk_score(self):
        ...
        self.results["criminal_exposure"] = list({
            v["statute"]: v for v in self.results["criminal_exposure"]
        }.values())
        
        self.results["civil_exposure"] = list({
            v["statute"]: v for v in self.results["civil_exposure"]
        }.values())
    ...
    def _create_executive_summary(self):
        ...
        if risk_score > 0.8:
            summary_parts.append("IMMEDIATE ACTION REQUIRED - Consult legal counsel within 24 hours.")
