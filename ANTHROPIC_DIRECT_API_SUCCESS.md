...
### Current .env Setup (placeholders only):
```ini
# ✅ Anthropic (Working - set locally)
ANTHROPIC_API_KEY=
ANTHROPIC_MODEL=claude-3-opus-20240229
ANTHROPIC_MAX_TOKENS=2048

# ⚠️ OpenAI (set locally)
OPENAI_API_KEY=
...
The primary OpenAI key in your local `.env` is showing 401 error. Update your local `.env` with a valid key (do not commit it). Ensure it is set as `OPENAI_API_KEY`.
