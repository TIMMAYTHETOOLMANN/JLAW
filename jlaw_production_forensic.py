...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="JLAW Production Forensic Analysis System (Benchmark-Compliant)",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('--ticker', type=str, default='NKE', help='Company ticker (default: NKE)')
    parser.add_argument('--cik', type=str, default='0000320187', help='Company CIK (default: 0000320187)')
    parser.add_argument('--year', type=int, default=2019, help='Analysis year (default: 2019)')
    parser.add_argument('--company', type=str, default='NIKE, Inc.', help='Company name')
    
    args = parser.parse_args()
    
    # Update global constants
    global NIKE_CIK, TARGET_COMPANY, ANALYSIS_YEAR
    NIKE_CIK = args.cik
    TARGET_COMPANY = args.company
    ANALYSIS_YEAR = args.year
    
    print(f"\n{'='*80}")
    print(f"JLAW PRODUCTION FORENSIC ANALYSIS")
    print(f"{'='*80}")
    print(f"Target: {TARGET_COMPANY} (CIK: {NIKE_CIK})")
    print(f"Period: {ANALYSIS_YEAR}")
    print(f"{'='*80}\n")
    ...
    async with UnifiedForensicAnalyzer() as analyzer:
        ...
        print(f"{'='*80}")
        print(f"Filings Analyzed:   {json_data['executive_summary']['total_filings_analyzed']}")
        print(f"Violations Found:   {json_data['executive_summary']['total_violations_identified']}")
        print(f"Criminal Referrals: {json_data['executive_summary']['criminal_referrals_recommended']}")
        print(f"Estimated Damages:  ${json_data['executive_summary']['estimated_total_damages']:,.2f}")
        print(f"{'='*80}\n")
