...
Every flagged $0 transaction gets classified into an investigable disposition:
    ...
    C) Needs conversion-terms reconciliation (Code C: price may be NA; missing terms => escalate)
...
Additionally, each transaction is enriched with:
    ...
    - Late filing repeat offender scoring with calendar + business day detail
    - Verbatim evidence snippets from footnotes flagging beneficial control retention
    - CrossFormReconciliationPanel for material ownership transfers (13D/13G + proxy checks)
"""
...
FSL_LABELS = {
    ...
    FSLDisposition.C_NEEDS_PRICE_RECON: "Needs conversion-terms reconciliation — price may be NA; missing terms => escalate",
...
PRICE_REQUIRED_BY_CODE = {
    ...
    'C': False,   # Conversion of derivative — price often NA; conversion TERMS required (see CONVERSION_TERMS_REQUIRED_BY_CODE)
...
# True = conversion terms (ratio, class, or description) MUST be present in footnotes/fields
# Used only for Code C (Conversion of derivative security).
CONVERSION_TERMS_REQUIRED_BY_CODE = {
    'C': True,    # Conversion — ratio/class/terms must be described; price field may legitimately be blank
}
...
def determine_price_required(transaction_code: str, is_derivative: bool) -> bool:
    """Determine whether a market price is required for this transaction type.

    Note: Code C (Conversion) does NOT require a market price — the price field
    may legitimately be blank/NA.  What IS required are conversion *terms*
    (ratio, class, description).  Use ``determine_conversion_terms_required``
    for Code C checks.
    """
    ...
    if required is not None:
        return required
    # Derivative exercises generally require a price (but C is handled above)
    if is_derivative and code in ('M', 'X', 'E'):
        return True
    return False


def determine_conversion_terms_required(transaction_code: str) -> bool:
    """Return True when the transaction code requires conversion terms in footnotes/fields."""
    return CONVERSION_TERMS_REQUIRED_BY_CODE.get(transaction_code.upper(), False)
...
class FootnoteClassification:
    ...
    # Verbatim sentences (≤ 2 sentences each) from footnotes that imply retained beneficial control.
    # Populated by classify_footnotes(); used as court-ready "Evidence snippet" fields.
    retained_control_snippets: List[str] = field(default_factory=list)
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "retained_control_snippets": self.retained_control_snippets,
        }
...
_FOOTNOTE_PATTERNS = {
    ...
    'beneficial_control_retained': re.compile(
        # Catches: "beneficial owner/ownership", "beneficially own",
        #          "indirectly own/held/control", "controlled by",
        #          "voting power/control", "retains/retained/retaining [beneficial] control/interest",
        #          "beneficial control", "deemed [to be] [the] beneficial [owner]"
        r'beneficial(?:ly)?\s+own|indirect(?:ly)?\s+(?:own|held|control)|'
        r'control(?:led)?\s+by|voting\s+(?:power|control)|retain(?:ed|s|ing)\s+(?:beneficial\s+)?(?:control|interest)|'
        r'beneficial\s+(?:owner(?:ship)?|control)|'
...
def classify_footnotes(footnotes: List[str]) -> FootnoteClassification:
    ...
    For ``beneficial_control_retained`` signals the function also extracts
    verbatim evidence snippets (≤ 2 sentences per footnote) and stores them in
    ``result.retained_control_snippets``.
    """
    ...
    # Extract verbatim sentences that imply retained beneficial control so they
    # can be used as court-ready "Evidence snippets".
    if result.beneficial_control_retained:
        bc_pattern = _FOOTNOTE_PATTERNS['beneficial_control_retained']
        for footnote in footnotes:
            fn_text = str(footnote).strip()
            # Split into individual sentences (rough split on '. ' / '.' at end)
            sentences = re.split(r'(?<=[.!?])\s+', fn_text)
            matching = [s.strip() for s in sentences if bc_pattern.search(s)]
            # Limit to first 2 matching sentences per footnote to keep snippets concise
            for snippet in matching[:2]:
                if snippet and snippet not in result.retained_control_snippets:
                    result.retained_control_snippets.append(snippet)
...
class FSLAssessment:
    ...
    # Code C-specific: True when conversion terms (ratio/class) are absent
    conversion_terms_missing: bool = False
    ...
    # Verbatim sentences extracted from footnotes that imply retained beneficial control
    retained_control_snippets: List[str] = field(default_factory=list)

    # Late filing — both calendar days and business days for full auditability
    is_late: bool = False
    days_late: int = 0            # Business days beyond the 2-day limit
    calendar_days_after_transaction: int = 0   # Raw calendar days from transaction to filing
    filing_date: str = ""         # ISO filing date for reference
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "conversion_terms_missing": self.conversion_terms_missing,
...
def classify_fsl(
    ...
    Decision tree (ordered by signal strength):
      C) Code C with missing conversion terms → conversion-terms reconciliation
         (plain price_required=True codes with price_source=missing → price reconciliation)
    ...
    conv_terms_req = determine_conversion_terms_required(code)
    ...
    # For Code C: conversion terms are missing when no conversion ratio/class found
    # in footnotes AND the exercise_price field is also absent.
    conv_terms_missing = False
    if conv_terms_req:
        has_conv_ratio = footnote_cls.conversion_ratio_present
        has_exercise_price = bool(txn_data.get('exercise_price'))
        conv_terms_missing = not has_conv_ratio and not has_exercise_price

    # Late filing: capture calendar days AND business days for auditability
    calendar_days = txn_data.get('calendar_days_after_transaction', 0)
    days_late_val = txn_data.get('days_late', 0)
    is_late_val = txn_data.get('is_late_filed', False)
    filing_date_val = txn_data.get('filing_date', '')
    ...
    assessment = FSLAssessment(
        ...
        conversion_terms_missing=conv_terms_missing,
        ...
        retained_control_snippets=list(footnote_cls.retained_control_snippets),
        is_late=is_late_val,
    ...
    # C) Conversion terms reconciliation (Code C) or price reconciliation (other codes)
    if conv_terms_req and conv_terms_missing:
        assessment.disposition = FSLDisposition.C_NEEDS_PRICE_RECON.value
        reasons.append(
            f"Code C (Conversion): price field may legitimately be blank/NA, "
            f"but conversion terms (ratio/class/description) are absent — escalate for terms reconciliation"
        )
        signal += 0.35
    elif price_req and price_src == PriceValueSource.MISSING:
        assessment.disposition = FSLDisposition.C_NEEDS_PRICE_RECON.value
    ...
    # D) Cross-form reconciliation
        ...
        if footnote_cls.beneficial_control_retained:
            ...
            for snippet in assessment.retained_control_snippets:
    ...
    if is_late_val:
        biz_days_total = days_late_val + 2  # total business days from transaction to filing
        if calendar_days > 0:
            reasons.append(
                f"Filed {calendar_days} calendar days after transaction date "
                f"({biz_days_total} business days); required within 2 business days "
                f"— {days_late_val} business day(s) late"
            )
        else:
            reasons.append(
                f"Filed {biz_days_total} business days after transaction date; "
                f"required within 2 business days — {days_late_val} business day(s) late"
            )
        signal += 0.10
...
# ═══════════════════════════════════════════════════════════════════
# CROSS-FORM RECONCILIATION PANEL
# ═══════════════════════════════════════════════════════════════════

# Threshold above which a Form 4 transfer should trigger 13D/13G scrutiny.
# SEC beneficial ownership rules require Schedule 13D/13G once a person or
# group crosses 5% of a class of equity securities (§13(d) / §13(g)).
# We also flag extremely large *individual* transfers even below 5% because
# multi-party cluster events can aggregate toward that threshold.
SCHEDULE_13DG_SHARE_THRESHOLD = 1_000_000   # Flag individual transfers ≥ 1M shares
SCHEDULE_13DG_VALUE_THRESHOLD = 50_000_000  # Flag individual transfers ≥ $50M implied value
PROXY_RECONCILIATION_TOLERANCE_PCT = 0.5   # Max ownership % delta before flagging discrepancy


@dataclass
class CrossFormReconciliationPanel:
    """
    Cross-form reconciliation for a single material Form 4 transfer (typically
    Code D or J with beneficial control retained or large share volume).

    Three required checks per the problem specification:
        1. ``schedule_13dg_expected`` — was a 13D/13G filing required given the
           magnitude of the transfer and the filer's beneficial owner status?
        2. ``schedule_13dg_observed`` — was a 13D/13G actually found in EDGAR
           for this filer around the same date?
        3. ``proxy_10k_reconciled`` — do the beneficial ownership tables in the
           proxy/DEF 14A or 10-K match the post-transfer state implied by the
           Form 4?

    A mismatch between expected and observed is a beacon of potential
    disclosure failure.
    """
    # Identity of the flagged transaction
    insider_name: str = ""
    transaction_date: str = ""
    transaction_code: str = ""
    shares: float = 0.0
    implied_value: float = 0.0   # shares × market price at transaction date (if available)
    security_title: str = ""

    # Check 1: Was 13D/13G expected?
    schedule_13dg_expected: bool = False
    schedule_13dg_expected_reason: str = ""

    # Check 2: Was 13D/13G observed in EDGAR?
    # Populated by the EDGAR lookup layer (node8 / edgar_client).
    # None = not yet checked; True = found; False = not found.
    schedule_13dg_observed: Optional[bool] = None
    schedule_13dg_filing_id: Optional[str] = None   # Accession number if found
    schedule_13dg_filing_date: Optional[str] = None

    # Check 3: Proxy / 10-K beneficial ownership table reconciled?
    # None = not yet checked; True = reconciled; False = discrepancy found.
    proxy_10k_reconciled: Optional[bool] = None
    proxy_discrepancy_detail: str = ""

    # Overall reconciliation verdict
    reconciliation_flag: bool = False   # True = at least one discrepancy found
    ...
    def to_dict(self) -> Dict[str, Any]:
        return {
            ...
            "security_title": self.security_title,
            "schedule_13dg_expected": self.schedule_13dg_expected,
            "schedule_13dg_expected_reason": self.schedule_13dg_expected_reason,
            "schedule_13dg_observed": self.schedule_13dg_observed,
            "schedule_13dg_filing_id": self.schedule_13dg_filing_id,
            "schedule_13dg_filing_date": self.schedule_13dg_filing_date,
            "proxy_10k_reconciled": self.proxy_10k_reconciled,
            "proxy_discrepancy_detail": self.proxy_discrepancy_detail,
            "reconciliation_flag": self.reconciliation_flag,
            "reconciliation_notes": self.reconciliation_notes,
        }

    def to_panel_text(self) -> str:
        """Render the panel as a human-readable block for inclusion in reports."""

        def yesno(val: Optional[bool], unknown: str = "NOT CHECKED") -> str:
            if val is None:
                return unknown
            return "YES" if val else "NO"

        lines = [
            f"  ┌─ Cross-Form Reconciliation Panel ─────────────────────────────",
            f"  │  Insider:        {self.insider_name}",
            f"  │  Date:           {self.transaction_date}  |  Code: {self.transaction_code}",
            f"  │  Shares:         {self.shares:,.0f}  |  Security: {self.security_title}",
        ]
        if self.implied_value:
            lines.append(f"  │  Implied Value:  ${self.implied_value:,.0f}")
        lines += [
            f"  ├─ 13D/13G Expected?   {yesno(self.schedule_13dg_expected, 'NO')}",
        ]
        if self.schedule_13dg_expected_reason:
            lines.append(f"  │    Reason: {self.schedule_13dg_expected_reason}")
        filing_ref = (
            f"{self.schedule_13dg_filing_id} ({self.schedule_13dg_filing_date})"
            if self.schedule_13dg_filing_id else "—"
        )
        lines += [
            f"  ├─ 13D/13G Observed?   {yesno(self.schedule_13dg_observed)}  |  Filing: {filing_ref}",
            f"  ├─ Proxy/10-K Reconciled?  {yesno(self.proxy_10k_reconciled)}",
        ]
        if self.proxy_discrepancy_detail:
            lines.append(f"  │    Detail: {self.proxy_discrepancy_detail}")
        if self.reconciliation_flag:
            lines.append(f"  ├─ ⚠  RECONCILIATION FLAG RAISED")
...
def build_cross_form_reconciliation_panel(
    assessment: FSLAssessment,
    schedule_13dg_filings: Optional[List[Dict[str, Any]]] = None,
    proxy_ownership_pct: Optional[float] = None,
    post_transfer_ownership_pct: Optional[float] = None,
    implied_value: float = 0.0,
) -> CrossFormReconciliationPanel:
    """
    Build a CrossFormReconciliationPanel for a single FSL assessment flagged as
    FSLDisposition.D_NEEDS_CROSS_FORM (entity transfer / beneficial control retained).

    Args:
        assessment: The FSL assessment for the flagged transaction.
        schedule_13dg_filings: Optional list of Schedule 13D/13G filing records
            (dicts with keys: accession_number, filing_date, filer_name,
            subject_cik, shares_owned, percent_owned) sourced from node8/EDGAR.
            Pass None to leave check 2 as "NOT CHECKED".
        proxy_ownership_pct: Reported beneficial ownership % from proxy/10-K.
            Pass None to leave check 3 as "NOT CHECKED".
        post_transfer_ownership_pct: Implied beneficial ownership % calculated
            from Form 4 post-transfer data. Used to compare against proxy.
        implied_value: Shares × market price at transaction date (optional).

    Returns:
        CrossFormReconciliationPanel with all three checks populated as far as
        the provided data allows.
    """
    panel = CrossFormReconciliationPanel(
        insider_name=assessment.insider_name,
        transaction_date=assessment.transaction_date,
        transaction_code=assessment.transaction_code,
        shares=assessment.shares,
        implied_value=implied_value,
        security_title=assessment.security_title,
    )

    notes: List[str] = []

    # ── Check 1: Is 13D/13G expected? ────────────────────────────────────────
    reasons_13dg = []
    if assessment.shares >= SCHEDULE_13DG_SHARE_THRESHOLD:
        reasons_13dg.append(
            f"Transfer of {assessment.shares:,.0f} shares meets or exceeds "
            f"{SCHEDULE_13DG_SHARE_THRESHOLD:,}-share materiality threshold"
        )
    if implied_value >= SCHEDULE_13DG_VALUE_THRESHOLD:
        reasons_13dg.append(
            f"Implied transfer value of ${implied_value:,.0f} meets or exceeds "
            f"${SCHEDULE_13DG_VALUE_THRESHOLD:,} materiality threshold"
        )
    if assessment.beneficial_control_retained_fn:
        reasons_13dg.append(
            "Footnote indicates beneficial control is retained post-transfer — "
            "filer may remain a 'beneficial owner' under Rule 13d-3"
        )
    if assessment.entity_transfer_fn:
        reasons_13dg.append(
            "Transfer to trust/LLC/partnership where filer likely retains "
            "voting/investment power — aggregated ownership may still apply"
        )

    panel.schedule_13dg_expected = bool(reasons_13dg)
    panel.schedule_13dg_expected_reason = "; ".join(reasons_13dg) if reasons_13dg else (
        "Transfer does not independently meet materiality thresholds"
    )

    # ── Check 2: Was 13D/13G observed? ───────────────────────────────────────
    if schedule_13dg_filings is not None:
        panel.schedule_13dg_observed = False
        for filing in schedule_13dg_filings:
            filer = str(filing.get('filer_name', '')).lower()
            insider_lower = assessment.insider_name.lower()
            if any(part in filer for part in insider_lower.split() if len(part) > 1):
                panel.schedule_13dg_observed = True
                panel.schedule_13dg_filing_id = str(filing.get('accession_number', ''))
                panel.schedule_13dg_filing_date = str(filing.get('filing_date', ''))
                break

        if panel.schedule_13dg_expected and not panel.schedule_13dg_observed:
            notes.append(
                "Form 4 indicates expected 13D/13G filing, but none observed in EDGAR — "
                "potential disclosure gap"
            )
            panel.reconciliation_flag = True
    # else: schedule_13dg_observed remains None (NOT CHECKED)

    # ── Check 3: Proxy / 10-K reconciliation ────────────────────────────────
    if proxy_ownership_pct is not None and post_transfer_ownership_pct is not None:
        delta = abs(proxy_ownership_pct - post_transfer_ownership_pct)
        panel.proxy_10k_reconciled = delta <= PROXY_RECONCILIATION_TOLERANCE_PCT
        if not panel.proxy_10k_reconciled:
            panel.proxy_discrepancy_detail = (
                f"Form 4 implied post-transfer ownership: {post_transfer_ownership_pct:.2f}%; "
                f"Proxy/10-K reported: {proxy_ownership_pct:.2f}%; "
                f"Delta: {delta:.2f}% — exceeds {PROXY_RECONCILIATION_TOLERANCE_PCT}% reconciliation tolerance"
            )
            notes.append(
                f"Beneficial ownership mismatch: Form 4 implies {post_transfer_ownership_pct:.2f}% "
                f"vs proxy/10-K {proxy_ownership_pct:.2f}% (delta {delta:.2f}%)"
            )
            panel.reconciliation_flag = True
    # else: proxy_10k_reconciled remains None (NOT CHECKED)

    panel.reconciliation_notes = notes
    return panel


def format_reconciliation_panels(panels: List[CrossFormReconciliationPanel]) -> str:
    """Format a section of Cross-Form Reconciliation Panels for inclusion in reports."""
    if not panels:
        return ""
    lines = [
        "",
        "=" * 80,
        "CROSS-FORM RECONCILIATION PANELS — MATERIAL OWNERSHIP TRANSFERS",
        "=" * 80,
    ]
    for panel in panels:
        lines.append(panel.to_panel_text())
    lines.append("")
    return "\n".join(lines)
...
def format_top_signals_report(top: List[FSLAssessment]) -> str:
    ...
    for i, a in enumerate(top, 1):
        ...
        if a.transaction_code == 'C':
            lines.append(f"  Conversion Terms Missing: {'YES' if a.conversion_terms_missing else 'No'}")
        lines.append(f"  Footnote Present: {'Yes' if a.footnote_present else 'NO'}  |  "
                      f"10b5-1 Mentioned: {'Yes' if a.mentions_10b5_1 else 'No'}")
        # Late filing detail — show both calendar and business days
        if a.is_late:
            biz_total = a.days_late + 2
            if a.calendar_days_after_transaction > 0:
                late_detail = (
                    f"YES — filed {a.calendar_days_after_transaction} calendar days after transaction date "
                    f"({biz_total} business days); required within 2 business days"
                )
            else:
                late_detail = (
                    f"YES — filed {biz_total} business days after transaction date; "
                    f"required within 2 business days"
                )
            lines.append(f"  Late Filed: {late_detail}")
        else:
            lines.append(f"  Late Filed: No")
        lines.append(f"  Repeat Offender: {'YES' if a.is_repeat_offender else 'No'}")
        ...
        # Show evidence snippets for beneficial control retained
        if a.retained_control_snippets:
            lines.append(f"  Beneficial Control Evidence Snippets:")
            for snippet in a.retained_control_snippets:
